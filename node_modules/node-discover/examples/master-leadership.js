...
var d = new Discover({ 
	...
	, mastersRequired : 2
	, leadershipElector : NullLeadership
});

function NullLeadership() {}

var steady = false;

setTimeout(() => steady = true, 1000);

d.on("added", function (node, obj, rinfo) {
	//node added
	if (steady) {
		d.emit('check')
	}
...
d.on("master", function (node, obj, rinfo) {
	//master added
	if (steady) {
		d.emit('check')
	}
...
d.on("check", function () {
	let leaders = countLeaders();

	leaders += d.me.isMaster ? 1 : 0;

	if (d.me.isMaster && leaders > 2 && iAmGreatest()) {
		d.demote();
	}
...
function countLeaders () {
	return Object.keys(d.nodes).reduce((a, key) => {
		a += d.nodes[key].isMaster ? 1 : 0
	}, 0);
}

function iAmGreatest () {
	return !Object.keys(d.nodes).some((key) => {
		d.me.weight < d.nodes[key].weight;
	});
}
